          ...
          <div class="content-main">
            ...
            <div v-if="!model.systemData.isWaitingReset">
              <div v-if="model.systemData.ruleSet !== 2">
                <div v-show="model.country.isHaveGyokuji" class="alert alert-info">この国は玉璽を持っています。<strong>{{ model.country.gyokujiGameDate.year + 10 * 12 }}</strong> 年 <strong>{{ model.country.gyokujiGameDate.month }}</strong> 月 までに所有すると統一勝利する可能性があります<br><span v-if="model.gameDate.year < 168"><br>玉璽勝利は、<strong>168</strong> 年 1 月以降発動します</span></div>
                <div class="alert alert-info">宗教勝利の進捗状況　都市: <strong>{{ model.countryReligionTownCount }}</strong> / <strong>{{ model.townCountForReligionWin }}</strong> 、国: <strong>1</strong> / <strong>{{ model.sameReligionCountryCount }}</strong><span v-if="!model.isAllTownHaveReligion"><br>まだすべての都市が宗教を信仰していません</span><span v-if="model.gameDate.year < 168"><br>宗教勝利は、<strong>168</strong> 年 1 月以降発動します</span></div>
              </div>
          ...
          <div class="setting-list">
            ...
            <div v-if="model.canCountryCommander" class="setting-row loading-container">
              ...
              <div>
                ...
                <button type="button" :class="{ 'btn': true, 'btn-outline-secondary': newCountryCommanderSubject !== 3 || newCountryCommanderSubjectData !== 11, 'btn-secondary': newCountryCommanderSubject === 3 && newCountryCommanderSubjectData === 11, }" @click="newCountryCommanderSubject = 3; newCountryCommanderSubjectData = 11; characterFilterMode = 1; model.updateCountryCharactersFiltering(newCountryCommanderSubject, newCountryCommanderSubjectData, 0)">宗教系</button><br>
      ...
      <div v-if="isOpenAddSecretaryDialog" class="dialog-body">
        ...
        <div class="dialog-content dialog-content-secretary-main">
          <div>
            ...
            <button :class="{ 'btn': true, 'btn-outline-secondary': selectedSecretaryType !== 32, 'btn-secondary': selectedSecretaryType === 32, }" @click="selectedSecretaryType = 32" v-if="model.canReligion && model.canSecretaryEvangelist">伝道師 (1)</button>
